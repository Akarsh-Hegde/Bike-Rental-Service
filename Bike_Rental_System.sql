...
-- bike category
insert into bike_category values
    ('ECONOMY', 5, 30, 0.9),
    ('COMPACT', 5, 32, 0.96),
    ('MID SIZE', 5, 35, 1.05),
    ('STANDARD', 5, 38, 1.14),
...
-- Bike Details
insert into bike_details( registration_no, model_name, make, model_year, mileage, category, bike_location) values
...
-- Queries
-- To fetch all available bikes
select * from bike_details, bike_category where category = category_name and available = true order by cost_per_day;

select * from bike_details inner join bike_category on category = category_name and available = true order by cost_per_day;

-- To fetch all bike category names
select category_name from bike_category;

-- To fetch all bikes by category 
select * from bike_details, bike_category where category = category_name and available = true order by cost_per_day where category_name = "economy";


-- Join Queries
-- 1. select all available bike details
select * from bike_details inner join bike_category on category = category_name where available = true order by cost_per_day;

-- 2. Show bike details along with its current location.
select * from bike_details inner join locations on bike_location = location_id;

-- 3. Show bikes booked by customers.
select Firstname,Lastname, model_name from customer_info join 
(booking_details join bike_details) where customer_info.Customer_ID = 
booking_details.Customer_ID and Registration_No = Bike_Reg_No;

-- 4.  Display Bike Details along with along with the category to which it belongs, No of persons it can hold, Cost per day
select * from bike_details inner join bike_category on category = category_name order by cost_per_day;
...
-- No. of bikes booked on particular date
select booking_date as OnDate, count(*) as BikesBooked from booking_details group by booking_date;

-- No of bikes belong to each category
select category, count(*) as NoOfBikes from bike_details group by category;

-- Set Operations
-- 1. Display the Customer First name and Last Name who have returned the Bike on or before the Return Date
...
-- 2. Display the Bike that were Booked and the Model_Year >2017
select Make,Model,Model_Year from bike_details join booking_details where 
Registration_No = Bike_Reg_No
EXCEPT
select Make,Model_name,Model_Year from bike_detail where Model_Year <= 2012;




-- Triggers
-- Trigger to update billing_details after bike return
...
begin
    ...
    if date(new.actual_return_date) > date(new.return_date) THEN
        ...
        select late_fee_per_hour into latefee from bike_details, bike_category where bike_details.category = bike_category.category_name and bike_details.registration_no = new.bike_reg_no;
        update billing_details set late_fee = latefee * extra_hours where booking_id = new.booking_id;
    end if; 

    update bike_details set available = true where registration_no = new.bike_reg_no;

end $$
delimiter ;

-- After insert into booking_details mark bike availability = false
...
update bike_details set available = false where registration_no = new.bike_reg_no;
...
-- create a prodecudre to dispalay the bike booked on specific date  with driver details
drop procedure if exists bike_booked_on_date;
delimiter $
create procedure bike_booked_on_date(in given_date date) 
begin
    select B.bike_reg_no, di.dl_number,di.driver_name, di.driver_dob, di.driving_experience, di.available, b.booking_date 
