...
from Read import getAllDrivers, getAllDriverContacts, getAllCustomers, getAllLocations, getAllBikeCategories, getAllBikes, getAllDiscountCoupons, AllBookingDetails
import pandas as pd
...
def DeleteBikeCategoryByNameUI():
    with st.expander("Before Deleting"):
        st.table(getAllBikeCategories())
    with st.container():
        selected_category = st.multiselect("Select Category Name to be deleted", [category["Name"] for category in getAllBikeCategories()])
        if st.button("Delete"):
            for category in selected_category:
                DeleteBikeCategoryByName(category)

def DeleteBikeCategoryByName(category_name):
    cursor.execute(f"use {DB_NAME}")
    query = ("delete from bike_category where category_name = %s")
...
def DeleteBikeRegNoUI():
    with st.expander("Before Deleting"):
        st.table(getAllBikes())
    with st.container():
        selected_regNo = st.multiselect("Select Reg No. to be deleted", options=[bike["Registration No"] for bike in getAllBikes()])
        if st.button("Delete"):
            for reg_no in selected_regNo:
                DeleteBikeRegNo(reg_no)

def DeleteBikeRegNo(reg_no):
    cursor.execute(f"use {DB_NAME}")
    query = ("delete from bike_details where registration_no = %s")
    cursor.execute(query, (reg_no,))
    db.commit()
    st.success("Bike details deleted successfully", icon="✅")
    with st.expander("After Deleting"):
