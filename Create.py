...
import pandas as pd
from Read import getAllDrivers, getAllBikeCategories, getAllLocations, getAllCustomers, getCustomerByUsername, getAllBikesWithCategory, getAllLocations, getLocationByName, getAllDiscountCoupons, getAVailableDrivers, getDiscountCouponByCode, getDriverCost, getDriverByDL
...
def AddCustomerUI():
    ...
    with registerContainer:
        ...
        st.button("Register", on_click=validateRegister, args=(userdata,))
...
def AddBikeCategoryUI():
    with st.container():
        ...
        if st.button("Add Bike Category"):
            # st.write(category)
            AddBikeCategory(category= category)

def AddBikeCategory(category):
    cursor.execute(f"use {DB_NAME}")
    query = (
        "insert into bike_category("
    ...
    db.commit()
    st.success('Bike Category Added Successfully!', icon="✅")
...
def AddBikeUI():
    with st.container():
        ...
        registration_no = registration_noUI.text_input("Bike Registration No.", max_chars= 6, placeholder="Bike Registration No.")
        ...
        category = categoryUI.selectbox("Bike Category", options=[category["Name"] for category in getAllBikeCategories()])
        location = bike_locationUI.selectbox("Bike Location", options=[location["Location Name"] for location in getAllLocations()])

        bike_details = {}
        bike_details["registration_no"] = registration_no
        ...
        bike_details["category"] = category
        bike_details["location"] = location


        if st.button("Add Bike"):
            # st.write(category)
            AddBike(bike_details= bike_details)

def AddBike(bike_details):
    # st.write(bike_details)
    cursor.execute(f"use {DB_NAME}")
    query = (
        "insert into bike_details("
        "registration_no, model_name, make, model_year, mileage, available, category, bike_location"
        ")"
    ...
    cursor.execute(query, (bike_details["registration_no"], bike_details["model_name"], bike_details["make"], bike_details["model_year"], bike_details["mileage"], bike_details["available"], bike_details["category"], locationId,))
    db.commit()
    st.success('Bike Added Successfully!', icon="✅")
...
def AddDiscountCouponUI():
    with st.container():
        ...
        coupon_code = coupon_codeUI.text_input("Coupon Code", placeholder="Coupon Code")
...
def getBikeFare(bikes, regNo, fromDate, toDate, discount):

     for bike in bikes:
        if bike["RegistrationNo"] == regNo:
            cost = NoOfDays(fromDate, toDate) * bike["CostPerDay"]
...
def AddBookingDetailsUI():
    ...
    available_bikes = getAllBikesWithCategory()
    with st.expander("Available bikes"):
        st.table(pd.DataFrame(available_bikes))
    ...
    bikeRegNo = bikeRegNoUI.selectbox("Select Bike Reg No.", [bike["RegistrationNo"] for bike in available_bikes])
    ...
    booking_data["basic_fare"] = getBikeFare(available_bikes, bikeRegNo, str(fromDate), str(toDate), booking_data["discount"])
...
def AddBookingDetails(booking_data):
    ...
    query = (
        ...
        "from_date, return_date, with_driver, booking_status, pickup_location, drop_location, coupon_code, bike_reg_no, customer_id, basic_fare, discount_percentage"
        ")"
